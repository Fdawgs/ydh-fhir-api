...
function buildBearerTokenRecord(result, req) {
	return {
		...
		access: {
			...
			hash: result.hash,
			salt: result.salt,
			scopes: secJSON.parse(result.scopes),
...
async function route(server, options) {
	...
	server.route({
		...
		handler: async (req, res) => {
			try {
				...
				let token = results?.recordsets?.[0];
		...
		handler: async (req, res) => {
			try {
				...
				if (whereArray.length === 0) {
					...
					const count = results?.recordsets?.[0]?.[0]?.total ?? 0;
					const tokens = server.cleanObject(
						results?.recordsets?.[1] ?? []
					);
	...
	server.route({
		...
		handler: async (req, res) => {
			try {
				/**
				 * Token prefixes are a clear way to make tokens identifiable, thus the 'ydhfhir'.
				...
				if (token && token.length > 0) {
					...
					const resObj = {
						...
						access: {
							token: key,
							scopes: secJSON.parse(token.scopes),
						},
					};
